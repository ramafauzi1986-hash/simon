...
<style>
.sidebar-menu .nav-link{color:#495057;border-radius:.5rem;padding:.55rem .75rem}
.sidebar-menu .nav-link:hover,.sidebar-menu .nav-link.active{background:#eaf2ff;color:#0d6efd}
.sidebar-submenu{border-left:2px solid #dee2e6;margin-left:1rem;padding-left:.5rem}
.sidebar-submenu .nav-link{font-size:.94rem}
.program-toggle .chevron{transition:transform .2s ease}.program-toggle[aria-expanded="true"] .chevron{transform:rotate(90deg)}
</style>
</head>
<body class="bg-light">
...
<a class="nav-link d-flex justify-content-between align-items-center program-toggle" href="#programMenu" data-bs-toggle="collapse" role="button" aria-expanded="{{request()->routeIs('programs.*','kegiatans.*','sub-kegiatans.*') ? 'true' : 'false'}}" aria-controls="programMenu">
<span>📁 Program</span><span class="chevron">›</span>
</a>
